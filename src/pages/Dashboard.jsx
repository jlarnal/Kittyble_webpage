...
function SetupPromptModal({ tank, toHexUid, onSetup, onDismiss, onLater }) {
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onLater}>
      <div className="bg-dark-card rounded-2xl p-5 w-full max-w-sm animate-in fade-in zoom-in-95" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-lg font-bold">Tank Setup Required</h3>
        </div>
        ...
        <div className="bg-dark-surface rounded-xl p-3 mb-4">
          ...
          <div className="text-sm font-semibold">{tank.name || 'Unnamed Tank'}</div>
          <div className="text-[10px] font-mono text-gray-500 uppercase">{toHexUid(tank.uid)}</div>
        </div>

        <p className="text-sm text-gray-400 mb-4">
          This tank needs to be configured with a name, density, and capacity before it can be used.
        </p>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {(data.tanks || []).map(tank => {
            ...
            const maxCapacityGrams = configured ? tank.capacity * tank.density * 1000 : 0
            const pct = configured ? Math.min(100, (tank.remainingWeight / maxCapacityGrams) * 100) : 0
              ...
              <div key={tank.uid} className="bg-dark-surface rounded-xl p-3 border border-white/5">
                <div className="flex justify-between items-start mb-2">
                  ...
                  {configured ? (
                    ...
                    <span className="text-[10px] bg-orange-600/80 rounded px-1.5 py-0.5 text-white">Not configured</span>
                  )}
                </div>
                <div className="mb-2">
                  <div className="text-sm font-semibold truncate leading-tight">{tank.name || 'Unnamed Tank'}</div>
                  {/* Hex UID Subtitle */}
                ...
                {configured ? (
                  ...
                  <div className="text-[10px] text-gray-500 italic">Set density & capacity to enable</div>
                )}
              </div>
            )
          })}
